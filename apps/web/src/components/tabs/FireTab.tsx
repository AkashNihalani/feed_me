...
const FIRE_BOOTSTRAP_PREFETCH_DAY_COUNT_MOBILE = 3;
...
function readValidFireCachedState(): FireCachedState | null {
  const cachedStateEntry = readCache<FireCachedState>(FIRE_STATE_CACHE_KEY);
  return cachedStateEntry
...
export default function FirePage() {
  ...
  const cursorRef = useRef(initialCachedState?.cursor ?? initialCachedCards.length);
  const fetchKeyRef = useRef('');
  const snapshotTokenRef = useRef<string | null>(
    typeof initialCachedState?.snapshotToken === 'string' && initialCachedState.snapshotToken.trim()
      ? initialCachedState.snapshotToken
      : null,
  );
  const backgroundPrefetchKeyRef = useRef<string | null>(null);
  const lastMetaRefreshAtRef = useRef(0);
  const cardsRef = useRef<FireAlertItem[]>(initialCachedCards);

  const [pickerDays, setPickerDays] = useState<string[]>(() => initialCachedState?.pickerDays || []);
  const [availableFeeds, setAvailableFeeds] = useState<FireFeedOption[]>(() => initialCachedState?.availableFeeds || []);
  const [availableCheckpoints, setAvailableCheckpoints] = useState<string[]>(() => sortCheckpointList(initialCachedState?.availableCheckpoints || []));
  const [warmupSummary, setWarmupSummary] = useState<WarmupSummary>(() => initialCachedState?.warmupSummary || {});
  const [cards, setCards] = useState<FireAlertItem[]>(initialCachedCards);
  const [loading, setLoading] = useState(!initialCachedState);
  const [deckMediaReady, setDeckMediaReady] = useState(false);
  const [cardsHydratedFromCache, setCardsHydratedFromCache] = useState(Boolean(initialCachedState));
  const [hasShownDeck, setHasShownDeck] = useState(initialCachedCards.length > 0);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [loadingMore, setLoadingMore] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [hasMore, setHasMore] = useState(Boolean(initialCachedState?.hasMore));
  const [total, setTotal] = useState(initialCachedState?.total || 0);
  const [selectedDay, setSelectedDay] = useState(initialCachedState?.selectedDay || '');
  const [isZSpaceOpen, setIsZSpaceOpen] = useState(false);
  const [filters, setFilters] = useState<FireFilterState>(() => (
    initialCachedState
      ? pruneFilters(initialCachedState.filters || createDefaultFireFilters(), initialCachedState.availableFeeds || [])
      : createDefaultFireFilters()
  ));
  const [headerHeight, setHeaderHeight] = useState(FIRE_MOBILE_HEADER_OFFSET);
  const [desktopModalCard, setDesktopModalCard] = useState<FireAlertItem | null>(null);
  const [isStandaloneMode, setIsStandaloneMode] = useState(false);
  const [isDesktopViewport, setIsDesktopViewport] = useState(false);
  const [initialDataReady, setInitialDataReady] = useState(Boolean(initialCachedState));
  ...
  useEffect(() => {
    ...
    const cachedState = readValidFireCachedState();
    ...
    return () => { mounted = false; };
  }, [handleUnauthorized, initialCachedState]);
  ...
  useEffect(() => {
    const node = mobileFilterButtonRef.current;
    if (!node) return undefined;
    const open = (event: Event) => {
      event.stopPropagation();
      openZSpaceFilter();
    };
    node.addEventListener('pointerdown', open, { capture: true });
    node.addEventListener('click', open, { capture: true });
    return () => {
      node.removeEventListener('pointerdown', open, { capture: true });
