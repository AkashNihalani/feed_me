...
-- Gate fire alerts to hot zone only: top 35%.
...
begin
  ...
  for r in
    ...
    where p.feeder_id = p_feeder_id
      ...
      and coalesce(pm.percentile_performance, 100)::int <= 35
  loop
...
-- Remove active v2 own-post alerts outside hot zone for Feb 17-21 IST.
update public.fire_alerts fa
set status = 'dismissed',
    updated_at = now()
where fa.status in ('new','sent')
  and fa.business_date_ist between date '2026-02-17' and date '2026-02-21'
  and fa.dedupe_key like 'v2:%'
  and coalesce(fa.signal_code,'') in ('slot','breakout','engagement','trajectory')
  and coalesce(fa.surface_percentile, 100) > 35;
