...
-- Hotfix: rival alerts require explicit anchor feeder role.
...
-- Deactivate bad rival alerts generated without explicit anchor (Feb 17-21 IST)
...
where fa.status in ('new','sent')
  ...
  and lower(coalesce(fa.signal_code,'')) = 'rival'
  and not exists (
    select 1
    from public.feeders fd
    where fd.feed_id = fa.feed_id
      and fd.status = 'active'
      and fd.role = 'anchor'
  );
