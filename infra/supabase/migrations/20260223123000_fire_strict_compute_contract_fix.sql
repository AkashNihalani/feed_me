...
begin
  ...
  -- Strict timing metrics from same hour cohorts.
  if new.post_hour_ist is not null then
    select round(percentile_cont(0.5) within group (order by pm.views))::bigint
    ...
    if v_mv is not null then
      select count(*) + 1 into v_hour_rank
      from public.post_metrics pm
      join public.posts p2 on p2.post_key = pm.post_key
      where p2.feeder_id = v_feeder_id
        and coalesce(p2.media_type,'') = coalesce(v_media_type,'')
      ...
      select count(*) + 1 into v_hour_pool
      from public.post_metrics pm
      join public.posts p2 on p2.post_key = pm.post_key
      where p2.feeder_id = v_feeder_id
        and coalesce(p2.media_type,'') = coalesce(v_media_type,'')
      ...
      if v_hour_pool > 0 then
        new.hour_percentile_feeder := greatest(1, least(100, round((v_hour_rank::numeric / v_hour_pool) * 100)));
      else
