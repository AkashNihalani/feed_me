-- Phase 1 Fire Engine: Layer 1 + Layer 5 resolver with idempotent dedupe

alter table public.fire_alerts
  add column if not exists dedupe_key text,
  add column if not exists alert_family text not null default 'velocity',
  add column if not exists alert_urgency text not null default 'today',
...
create index if not exists fire_alerts_post_checkpoint_idx
  on public.fire_alerts(post_key, checkpoint, created_at desc);

-- Keep checks additive and idempotent.
do $$
begin
  if not exists (
    select 1
    ...
    alter table public.fire_alerts
      ...
      check (alert_family in ('velocity','competitive','intelligence'));
  end if;
  ...
  if not exists (
    ...
    alter table public.fire_alerts
      add constraint fire_alerts_alert_urgency_check
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'fire_alerts_checkpoint_check'
      and conrelid = 'public.fire_alerts'::regclass
  ) then
    alter table public.fire_alerts
      add constraint fire_alerts_checkpoint_check
      check (checkpoint is null or checkpoint in ('d1','d2','d3','d7','d21'));
  end if;
  ...
  if not exists (
    ...
    alter table public.fire_alerts
      add constraint fire_alerts_confidence_check
      check (confidence is null or confidence in ('early','confirmed'));
  end if;
end $$;

create or replace function public.fn_fire_alert_type_from_percentile_tag(p_tag text)
returns text
language sql
immutable
as $$
  select case
    when p_tag = '🚀' then 'blaze'
    when p_tag = '🔥' then 'burn'
    when p_tag = '✅' then 'spark'
    else null
  end;
$$;

create or replace function public.fn_fire_alert_urgency_from_type(p_type text)
returns text
language sql
immutable
as $$
  select case
    when p_type = 'blaze' then 'now'
    when p_type = 'burn' then 'today'
    when p_type = 'spark' then 'watch'
    else 'today'
  end;
$$;
...
begin
  ...
  if v_checkpoint = 'd1' then
    ...
    v_body := format(
      '@%s entered %s at D1. Pattern is separating from baseline. Keep this creative direction active.',
      coalesce(v_handle, 'handle'),
      v_tag
    );
    ...
    v_body := format(
      '@%s is accelerating (%s%% better since D1). Momentum is strengthening post-publish.',
      coalesce(v_handle, 'handle'),
      v_delta
    );
  elsif v_checkpoint in ('d3','d7') and v_delta is not null and v_delta < 0 then
    ...
    v_body := format(
      '@%s cooled after launch (%s%% weaker vs D1). Treat this as soft momentum and avoid over-scaling yet.',
      coalesce(v_handle, 'handle'),
      abs(v_delta)
    );
  else
    ...
    v_body := format(
      '@%s is in hot zone at %s. Monitor next checkpoint for momentum confirmation.',
      coalesce(v_handle, 'handle'),
      upper(v_checkpoint)
    );
  end if;
...
create or replace function public.tg_enqueue_phase1_fire_alerts()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  perform public.enqueue_phase1_fire_alert(new.post_key, new.checkpoint);
  return new;
end;
$$;
...
create trigger trg_enqueue_phase1_fire_alerts
  after insert or update of percentile_tag, percentile_performance, metric_value, computed_at
  on public.post_metrics
  for each row
  execute function public.tg_enqueue_phase1_fire_alerts();
