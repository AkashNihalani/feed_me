...
-- Source-of-truth schedule contract for the Apify ingestion lane.
-- Compute + fire alerts remain untouched; only scrape timing / queue semantics
-- are normalized here.

-- Inclusive checkpoint semantics:
-- D1 = post day, D3 = post day + 2, D7 = post day + 6, D21 = post day + 20.
-- All checkpoint jobs are due at 18:30 IST so the worker can batch them into a
-- single evening checkpoint scrape.
...
-- Keep checkpoint queue behavior self-healing: due jobs are always revived as
-- pending, and historical misses are immediately catch-up eligible instead of
-- being stranded as skipped rows.
...
-- Daily feeder enqueue stays anchored to the previous IST business day because
-- the scrape itself runs just after midnight IST.
...
-- Keep the legacy worker + any residual transitional rows compatible.
...
-- Repair recent open checkpoint jobs so their due times match 18:30 IST.
...
-- Ensure existing recent posts participate in the evening checkpoint lane even
-- if jobs were never enqueued during an earlier pipeline mismatch.
select public.backfill_missing_checkpoint_jobs(45);

-- Scrape the previous day at 00:05 IST, with two watchdogs to keep the
-- self-healing repair lane intact.
do $$
declare
  r record;
begin
