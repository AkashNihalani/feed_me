-- Fix 1: median_delta robustness in feeder baselines
-- Fix 2: ensure missing checkpoint jobs are backfilled for existing posts
...
begin
  ...
  select
    ...
    'pending',
    case cp.checkpoint
      when 'd3' then public.fn_checkpoint_due_at(p.posted_at, 3, p_tz, p_hour, p_minute)
      when 'd7' then public.fn_checkpoint_due_at(p.posted_at, 7, p_tz, p_hour, p_minute)
      when 'd21' then public.fn_checkpoint_due_at(p.posted_at, 21, p_tz, p_hour, p_minute)
    end
  from public.posts p
...
-- One-time backfill for recent posts so D3/D7/D21 progression starts if rows were missing.
select public.backfill_missing_checkpoint_jobs(30);

-- One-time baseline refresh with new median_delta logic.
do $$
declare
  r record;
  cp text;
