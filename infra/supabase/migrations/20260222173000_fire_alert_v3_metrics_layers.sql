...
alter table public.post_metrics
  add column if not exists views_rank_feed int,
  add column if not exists likes_rank_feed int,
  add column if not exists comments_rank_feed int,
  add column if not exists best_in_last_n_views int,
  add column if not exists best_in_last_n_likes int,
  add column if not exists best_in_last_n_comments int;
...
begin
  ...
  loop
    v_views_multiple := coalesce(r.views_multiple, case when nullif(coalesce(r.baseline_views, 0), 0) is null then null else coalesce(r.views,0)::numeric / nullif(coalesce(r.baseline_views, 0), 0)::numeric end);
    v_likes_multiple := coalesce(r.likes_multiple, case when nullif(coalesce(r.baseline_likes, 0), 0) is null then null else coalesce(r.likes,0)::numeric / nullif(coalesce(r.baseline_likes, 0), 0)::numeric end);
    v_comments_multiple := coalesce(r.comments_multiple, case when nullif(coalesce(r.baseline_comments, 0), 0) is null then null else coalesce(r.comments,0)::numeric / nullif(coalesce(r.baseline_comments, 0), 0)::numeric end);
    ...
    v_surface_delta := case when v_cp='d1' then null else (v_d1 - coalesce(case when v_cp='d3' then v_d3 when v_cp='d7' then v_d7 else v_surface_pct end, v_surface_pct)) end;
...
-- Recompute new per-metric fields for existing rows.
update public.post_metrics
set likes = likes
where computed_at >= now() - interval '30 days';

-- Backfill fire v3 rows for active feeders over recent window.
do $$
declare
  d date;
  r record;
  cp text;
  v_from date := date '2026-02-17';
...
begin
  for d in select gs::date from generate_series(v_from, v_to, interval '1 day') gs loop
    for r in select id from public.feeders where status = 'active' loop
      foreach cp in array array['d1','d3','d7'] loop
