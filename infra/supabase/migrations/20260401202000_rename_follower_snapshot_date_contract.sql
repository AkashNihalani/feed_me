...
alter table if exists public.feeder_follower_snapshots
  rename column week_start_ist to snapshot_date_ist;

alter index if exists public.feeder_follower_snapshots_week_idx
  rename to feeder_follower_snapshots_snapshot_date_idx;
...
begin
  insert into public.feeder_follower_snapshots (feeder_id, snapshot_date_ist, follower_count, captured_at)
  values (new.id, v_business_date, v_count, now())
  on conflict (feeder_id, snapshot_date_ist) do update
...
begin
  ...
  if v_anchor_feeder_id is not null then
    select coalesce(
      json_agg(
        json_build_object(
          'snapshot_date_ist', s.snapshot_date_ist,
    ...
    where s.feeder_id = v_anchor_feeder_id
      and s.snapshot_date_ist >= v_window_start
      and s.snapshot_date_ist < v_window_end;
  else
    select coalesce(
      json_agg(
        json_build_object(
    ...
    into v_ascent_series
    from (
      select s.snapshot_date_ist, sum(s.follower_count)::bigint as follower_count
      from public.feeder_follower_snapshots s
