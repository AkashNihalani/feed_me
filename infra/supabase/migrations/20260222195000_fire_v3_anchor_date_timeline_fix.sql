...
begin
  ...
  loop
    v_anchor_day := coalesce(r.anchor_business_date_ist, r.captured_business_date_ist, v_day);
    ...
    v_payload := jsonb_build_object(
      ...
      'meta', jsonb_build_object(
        ...
        'anchor_business_date_ist', v_anchor_day,
        'captured_business_date_ist', r.captured_business_date_ist
      )
    );

    v_dedupe := format('v3:%s:%s:%s', r.post_key, v_cp, v_anchor_day);

    perform public.fn_fire_upsert_row(
      v_dedupe, v_feed_id, p_feeder_id, r.post_key, v_cp, v_anchor_day,
...
-- Normalize existing v3 timeline rows onto anchor day and dedupe by anchor day key.
with tagged as (
  select
    fa.id,
    fa.created_at,
    fa.post_key,
    lower(coalesce(fa.checkpoint, 'd1')) as cp,
    coalesce(nullif(fa.payload->'meta'->>'anchor_business_date_ist','')::date, fa.business_date_ist) as anchor_day
  from public.fire_alerts fa
  where fa.dedupe_key like 'v3:%'
), ranked as (
  select
    t.*,
    format('v3:%s:%s:%s', t.post_key, t.cp, t.anchor_day) as target_dedupe,
    row_number() over (
      partition by t.post_key, t.cp, t.anchor_day
...
set
  business_date_ist = t.anchor_day,
  dedupe_key = format('v3:%s:%s:%s', t.post_key, t.cp, t.anchor_day),
  updated_at = now()
from tagged t
where fa.id = t.id
  and (
    fa.business_date_ist is distinct from t.anchor_day
    or fa.dedupe_key is distinct from format('v3:%s:%s:%s', t.post_key, t.cp, t.anchor_day)
  );
